...
def train_neuron(features: np.ndarray, labels: np.ndarray, initial_weights: np.ndarray, initial_bias: float, learning_rate: float, epochs: int) -> (np.ndarray, float, list[float]):
    features, labels, weights = np.array(features), np.array(labels), np.array(initial_weights)
    mse_values = []
    bias = initial_bias
...
updated_weights, updated_bias, mse_losses = train_neuron(features, labels, initial_weights, initial_bias, learning_rate, epochs)
